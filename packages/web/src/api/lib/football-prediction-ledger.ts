...
export function recordFootballPrediction(row: Omit<FootballPredictionLedgerRow, "recordedAt"> & { recordedAt?: string }): FootballPredictionLedgerRow {
  ...
  if (recordedDay > fixtureDay) throw new Error("prediction recorded after fixture date");
  if (!row.predictionId || !row.home || !row.away || row.home === row.away) throw new Error("prediction identity is invalid");
  const total = row.probHome + row.probDraw + row.probAway;
  if (![row.probHome, row.probDraw, row.probAway].every(Number.isFinite) || row.probHome < 0 || row.probDraw < 0 || row.probAway < 0 || Math.abs(total - 1) > 1e-6) {
    throw new Error("prediction probabilities must be finite, non-negative, and sum to 1");
  }
  return { ...row, recordedAt };
}

export function resolveFootballPrediction(row: FootballPredictionLedgerRow, outcome: 0 | 1 | 2): FootballPredictionLedgerRow {
...
export function reportFootballLedger(rows: FootballPredictionLedgerRow[], minimumResolved = 100): FootballLedgerReport {
  const resolved = rows.filter((r) => r.outcome != null).map((r): ThreeWaySample => ({ probHome: r.probHome, probDraw: r.probDraw, probAway: r.probAway, outcome: r.outcome! }));
  let correct = 0;
  ...
  const calibration = topProbabilityCalibration(resolved);
  return {
    total: rows.length,
