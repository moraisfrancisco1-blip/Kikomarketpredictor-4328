...
export function fitDixonColes(matches: DCMatch[], opts: { halfLifeDays?: number; iterations?: number } = {}): FittedModel {
  ...
  for (const m of sorted) { teams.add(m.home); teams.add(m.away); }
  ...
  for (let it = 0; it < iters; it++) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const m = sorted[i], neu = !!m.neutral;
      const fH = Math.exp(intercept + def[m.away] + (neu ? 0 : homeAdv));
      numA[m.home] += w[i] * m.hg; denA[m.home] += w[i] * fH;
      numD[m.away] += w[i] * m.hg; denD[m.away] += w[i] * Math.exp(intercept + att[m.home] + (neu ? 0 : homeAdv));
      const fA = Math.exp(intercept + def[m.home]);
      numA[m.away] += w[i] * m.ag; denA[m.away] += w[i] * fA;
      numD[m.home] += w[i] * m.ag; denD[m.home] += w[i] * Math.exp(intercept + att[m.away]);
    }
    ...
    let nH = 0, dH = 0;
    for (let i = 0; i < sorted.length; i++) {
      const m = sorted[i]; if (m.neutral) continue;
      nH += w[i] * m.hg;
  ...
  for (let r = -0.18; r <= 0.000001; r += 0.01) {
    let ll = 0;
    for (let i = 0; i < sorted.length; i++) {
      const m = sorted[i], neu = !!m.neutral;
...
export type DCPrediction = {
  expHomeGoals: number; expAwayGoals: number; probHome: number; probDraw: number; probAway: number;
  over25: number; under25: number; bttsYes: number; topScores: { score: string; prob: number }[];
};
...
export type PredictOpts = {
  ...
  restHomeDays?: number;
  restAwayDays?: number;
  homeAttackMult?: number; homeDefMult?: number; awayAttackMult?: number; awayDefMult?: number;
...
export function predictMatch(model: FittedModel, home: string, away: string, opts: PredictOpts = {}): DCPrediction {
  ...
  const ah = model.att[home] ?? 0, dh = model.def[home] ?? 0, aa = model.att[away] ?? 0, da = model.def[away] ?? 0;
  ...
  const MAX = 10; let pH = 0, pD = 0, pA = 0, over = 0, btts = 0;
  ...
  return {
    expHomeGoals: +lh.toFixed(2), expAwayGoals: +la.toFixed(2),
    probHome: pH / norm, probDraw: pD / norm, probAway: pA / norm,
    over25: over / norm, under25: 1 - over / norm, bttsYes: btts / norm,
    topScores: scoreList.slice(0, 5).map(s => ({ score: s.score, prob: +(s.prob / norm).toFixed(4) })),
  };
}

// Deprecated compatibility helper. Production prediction paths must pass an
// explicit fixture date and use restDaysAt() instead of the current date.
export function restDays(model: FittedModel, team: string): number | undefined {
  return restDaysAt(model, team, new Date().toISOString());
}

export function restDaysAt(model: FittedModel, team: string, targetDate: string): number | undefined {
  const last = model.lastDate[team];
  if (!last) return undefined;
  const targetMs = new Date(targetDate).getTime(), lastMs = new Date(last).getTime();
  if (!Number.isFinite(targetMs) || !Number.isFinite(lastMs) || lastMs >= targetMs) return undefined;
  return Math.max(0, Math.round((targetMs - lastMs) / 86_400_000));
}

export type Backtest = {
  n: number; brier: number; logLoss: number; accuracy: number;
  baselineBrier: number; baselineLogLoss: number; refitEvery: number; testFrom: string;
};
...
export function backtest(matches: DCMatch[], opts: { halfLifeDays?: number; testFraction?: number; refitEvery?: number; minTrain?: number } = {}): Backtest {
  const sorted = [...matches].sort((a, b) => (a.date < b.date ? -1 : 1));
  const testFraction = opts.testFraction ?? 0.25, refitEvery = opts.refitEvery ?? 20, minTrain = opts.minTrain ?? 120;
  const startIdx = Math.max(minTrain, Math.floor(sorted.length * (1 - testFraction)));
  if (startIdx >= sorted.length - 1) return { n: 0, brier: 0, logLoss: 0, accuracy: 0, baselineBrier: 0, baselineLogLoss: 0, refitEvery, testFrom: sorted[startIdx]?.date ?? "" };
  let bh = 0, bd = 0, ba = 0;
  for (let i = 0; i < startIdx; i++) { const m = sorted[i]; if (m.hg > m.ag) bh++; else if (m.hg === m.ag) bd++; else ba++; }
  const bt = bh + bd + ba || 1, base = [bh / bt, bd / bt, ba / bt];
  let model = fitDixonColes(sorted.slice(0, startIdx), { halfLifeDays: opts.halfLifeDays });
  let sinceFit = 0, brier = 0, logLoss = 0, correct = 0, baseBrier = 0, baseLogLoss = 0, n = 0;
  for (let i = startIdx; i < sorted.length; i++) {
    if (sinceFit >= refitEvery) { model = fitDixonColes(sorted.slice(0, i), { halfLifeDays: opts.halfLifeDays }); sinceFit = 0; }
    sinceFit++;
    const m = sorted[i];
    if (model.att[m.home] == null || model.att[m.away] == null) continue;
    const restHomeDays = restDaysAt(model, m.home, m.date), restAwayDays = restDaysAt(model, m.away, m.date);
    const pr = predictMatch(model, m.home, m.away, { neutral: m.neutral, restHomeDays, restAwayDays });
    const p = [pr.probHome, pr.probDraw, pr.probAway];
    const outcome = m.hg > m.ag ? 0 : m.hg === m.ag ? 1 : 2;
    const y = [0, 0, 0]; y[outcome] = 1;
    for (let k = 0; k < 3; k++) { brier += (p[k] - y[k]) ** 2; baseBrier += (base[k] - y[k]) ** 2; }
    logLoss += -Math.log(Math.max(1e-9, p[outcome])); baseLogLoss += -Math.log(Math.max(1e-9, base[outcome]));
    if (p.indexOf(Math.max(...p)) === outcome) correct++;
    n++;
  }
  return {
    n, brier: n ? +(brier / n).toFixed(4) : 0, logLoss: n ? +(logLoss / n).toFixed(4) : 0,
    accuracy: n ? +(correct / n).toFixed(4) : 0, baselineBrier: n ? +(baseBrier / n).toFixed(4) : 0,
    baselineLogLoss: n ? +(baseLogLoss / n).toFixed(4) : 0, refitEvery, testFrom: sorted[startIdx].date,
  };
}

export type TuneResult = { halfLifeDays: number; logLoss: number; brier: number; candidates: { halfLifeDays: number; logLoss: number }[] };
export function tuneHalfLife(matches: DCMatch[], candidates: number[] = [90, 135, 180, 270, 400], btOpts: { testFraction?: number; refitEvery?: number } = {}): TuneResult {
  let best: TuneResult | null = null; const tried: { halfLifeDays: number; logLoss: number }[] = [];
  for (const hl of candidates) {
    const bt = backtest(matches, { halfLifeDays: hl, testFraction: btOpts.testFraction ?? 0.3, refitEvery: btOpts.refitEvery ?? 30 });
    if (!bt.n) continue;
...
export function predictOU(lh: number, la: number, rho: number): OUResult {
  const MAX = 12, lines = [0.5, 1.5, 2.5, 3.5, 4.5], over: number[] = new Array(lines.length).fill(0);
  let total = 0;
  for (let i = 0; i <= MAX; i++) for (let j = 0; j <= MAX; j++) { const p = poissonPmf(i, lh) * poissonPmf(j, la) * tau(i, j, lh, la, rho); total += p; for (let k = 0; k < lines.length; k++) if (i + j > lines[k]) over[k] += p; }
  return lines.map((line, k) => ({ line, over: total > 0 ? +(over[k] / total).toFixed(4) : 0.5, under: total > 0 ? +(1 - over[k] / total).toFixed(4) : 0.5 }));
}
...
export function predictHandicap(lh: number, la: number, rho: number, line: number): HandicapResult {
  const MAX = 12; let homeCovers = 0, awayCovers = 0, push = 0, total = 0;
  for (let i = 0; i <= MAX; i++) for (let j = 0; j <= MAX; j++) { const p = poissonPmf(i, lh) * poissonPmf(j, la) * tau(i, j, lh, la, rho); total += p; const margin = (i - j) - line; if (margin > 0) homeCovers += p; else if (margin < 0) awayCovers += p; else push += p; }
  return { line, homeCovers: total > 0 ? +(homeCovers / total).toFixed(4) : 0, awayCovers: total > 0 ? +(awayCovers / total).toFixed(4) : 0, push: total > 0 ? +(push / total).toFixed(4) : 0 };
}
...
export function predictAccumulator(legs: { prob: number; pick: string }[]): AccResult {
  let combined = 1; for (const leg of legs) combined *= leg.prob;
  return { combinedProb: +combined.toFixed(6), fairDecimalOdds: combined > 0 ? +(1 / combined).toFixed(2) : 0, legs: legs.map(l => ({ pick: l.pick, prob: +l.prob.toFixed(4) })) };
}
...
export function backtestWithSamples(matches: DCMatch[], opts: { halfLifeDays?: number; testFraction?: number; refitEvery?: number; minTrain?: number } = {}): WFSample[] {
  const sorted = [...matches].sort((a, b) => (a.date < b.date ? -1 : 1));
  const testFraction = opts.testFraction ?? 0.5, refitEvery = opts.refitEvery ?? 25, minTrain = opts.minTrain ?? 120;
  const startIdx = Math.max(minTrain, Math.floor(sorted.length * (1 - testFraction)));
  ...
  const updateElo = (home: string, away: string, hg: number, ag: number) => {
    const eH = getElo(home), eA = getElo(away), expH = 1 / (1 + Math.pow(10, (eA - eH) / 400));
    const actH = hg > ag ? 1 : hg === ag ? 0.5 : 0;
    eloRating[home] = eH + K * (actH - expH); eloRating[away] = eA + K * ((1 - actH) - (1 - expH));
  };
  for (let i = 0; i < startIdx; i++) { const m = sorted[i]; updateElo(m.home, m.away, m.hg, m.ag); }
  let model = fitDixonColes(sorted.slice(0, startIdx), { halfLifeDays: opts.halfLifeDays });
  let sinceFit = 0; const samples: WFSample[] = [];
  for (let i = startIdx; i < sorted.length; i++) {
    if (sinceFit >= refitEvery) { model = fitDixonColes(sorted.slice(0, i), { halfLifeDays: opts.halfLifeDays }); sinceFit = 0; }
    sinceFit++; const m = sorted[i];
    if (model.att[m.home] == null || model.att[m.away] == null) { updateElo(m.home, m.away, m.hg, m.ag); continue; }
    const pr = predictMatch(model, m.home, m.away, { neutral: m.neutral, restHomeDays: restDaysAt(model, m.home, m.date), restAwayDays: restDaysAt(model, m.away, m.date) });
    const outcome: 0 | 1 | 2 = m.hg > m.ag ? 0 : m.hg === m.ag ? 1 : 2;
    samples.push({ date: m.date, home: m.home, away: m.away, hg: m.hg, ag: m.ag, probHome: +pr.probHome.toFixed(4), probDraw: +pr.probDraw.toFixed(4), probAway: +pr.probAway.toFixed(4), eloHome: Math.round(getElo(m.home)), eloAway: Math.round(getElo(m.away)), outcome });
